...
Chunk* createChunk(const int numRecords, const int numFields) {
    Chunk* chunk = malloc(sizeof(Chunk));
    Record* records = calloc(numRecords, sizeof(Record));
    // Allocate a contiguous block of memory for all fields of all records, calloc applied to avoid any initialization MFLAG
    INT_128* fieldBlock = calloc(numRecords * numFields, sizeof(INT_128));
...
JoinBuffer* createJoinBuffer(const int numFields) {
    ...
    //loop unrolling MFLAG
    int i = 0;
    for (i; i + 3 < numRecords; i += 4) { 
        initRecord(&records[i], numFields);
        initRecord(&records[i + 1], numFields);
        initRecord(&records[i + 2], numFields);
        initRecord(&records[i + 3], numFields);
    }
...
static inline bool recordFitsBuffer(const Record* record, const int key_index, const JoinBuffer* buffer) {
    ...
    // removing unnecessary state variable MFLAG
    // int compare = (buffer->key == record->fields[key_index]);
    // return compare;

    return (buffer->key == record->fields[key_index]);
}
...
void writeRecordToBuffer(const Record* record, JoinBuffer* buffer, const int right_on) {
    ...
    //i tried loop urooling with 1,2,3 itertions but it didnt get any improvement so I just left the last version of it MFLAG
    // int i = 0;
    // for (; i + 1 < buffer->numFields; i += 2) {
    //     copyField(&buffer->records[buffer->numRecords], record, i, i);
    //     copyField(&buffer->records[buffer->numRecords], record, i + 1, i + 1);
        
        
    // }

    // for (; i < buffer->numFields; i++) {
    //     copyField(&buffer->records[buffer->numRecords], record, i, i);
    // }
    buffer->numRecords++;
}
...
static inline void loadLineIntoRecord(const char* line, const Record* record, const int numFields) {
    int field_start = 0;
    int current_field = 0;
    int max_length = numFields * FIELD_LENGTH; //precomputing limit MFLAG

    for(int i = 0; i < max_length & line[i] != '\n'; i++) {
        // Found end of current field
        if(line[i] == ',') {
            record->fields[current_field] = encodeField(&line[field_start]);
            field_start = i+1;
...
Table* joinTables(Table* left, Table* right, const char* outputFileName) {
    ...
    if(!left->endOfFile){//if plus do-while instead of while MFLAG
        do{
            bool rightEOF = right->endOfFile;
            int comparison = rightEOF ? -1 : compareRecordsOnFields(&left->currentRecord, &right->currentRecord, 3, 0);
            ...
            if(comparison == 0) {
                ...
                int numRecords = buffer->numRecords; //not to calculate every time MFLAG
                if(recordFitsBuffer(&left->currentRecord, 3, buffer)) {
                    // In this case we can match the left record to all previous right records that were loaded
                    // into the buffer. For each of the Records in the buffer output a join result of the current left
                    // row with one previous right row.
...
Table* joinThreeTables(Table* left, Table* middle, Table* right,
    ...
    if (!left->endOfFile){
        do{
            //if plus do while instead of while MFLAG
            ...
            if(l_greater_m == 0 && l_greater_r == 0) {  // case (B B B), all active
                ...
                if(recordFitsBuffer(&middle->currentRecord, 0, rightBuffer)) {
                    ...
                    // into the buffer. For each of the Records in the buffer output a join result of the current left
                    // row and middle row with one previous right row.
                    //loop unrolling  wont be good solution since this isn't trivious loop MFLAG
                    for(int i = 0; i < rightBuffer->numRecords; i++) {
                        processThreeRecords(&left->currentRecord,
                                                    &middle->currentRecord,
                                                    &rightBuffer->records[i],
    ...
    //I trieds to unroll loop not good solution MFLAG
    for(int i; i < numChunks; i++) {

...
static inline void _mergeTwoRecords(const Table* left, const Table* right, const int right_offset, // INT_128* rightFields,
                        const Record* target) {
    copyField(target, &left->currentRecord, 0, 3 ); // D
    copyField(target, &left->currentRecord, 1, 0); // A
    copyField(target, &left->currentRecord, 2, 1); // B
    copyField(target, &left->currentRecord, 3, 2); // C
    target->fields[4] = peekPreviousField(right, right_offset, 1);      // E
}
...
int main(int argc, char* argv[]) {
    ...
    char output_filename[] = "output-V2.2.csv";

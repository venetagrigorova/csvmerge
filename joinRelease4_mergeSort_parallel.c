...
static void parallelMergesortOnZero(Record* arr, int left, int right, Record* temp, int depthLimit) {
    if (left >= right) {
        return;
    }
    ...
    if (depthLimit > 0) {
    #pragma omp task shared(arr, temp) firstprivate(left, mid, depthLimit)
        {
            parallelMergesortOnZero(arr, left, mid, temp, depthLimit - 1);
        }
    #pragma omp task shared(arr, temp) firstprivate(mid, right, depthLimit)
        {
            parallelMergesortOnZero(arr, mid + 1, right, temp, depthLimit - 1);
        }
    #pragma omp taskwait
    } else {
        if (left < mid) {
            parallelMergesortOnZero(arr, left, mid, temp, 0);
        }
        if (mid + 1 < right) {
            parallelMergesortOnZero(arr, mid + 1, right, temp, 0);
        }
    }
...
void parallelSortOnZero(Record* arr, int n) {
    ...
    Record* temp = malloc(sizeof(Record) * n);
    if (!temp) {
...
#pragma omp single
        {
            parallelMergesortOnZero(arr, 0, n - 1, temp, depthLimit);
        }
    }
    free(temp);
}

static void parallelMergesortOnThird(Record* arr, int left, int right, Record* temp, int depthLimit) {
    if (left >= right) {
        return;
    }
    int mid = left + (right - left) / 2;
    ...
    #pragma omp task shared(arr, temp) firstprivate(left, mid, depthLimit)
        {
            parallelMergesortOnThird(arr, left, mid, temp, depthLimit - 1);
        }
    #pragma omp task shared(arr, temp) firstprivate(mid, right, depthLimit)
        {
            parallelMergesortOnThird(arr, mid + 1, right, temp, depthLimit - 1);
        }
    #pragma omp taskwait
    } else {
        if (left < mid) {
            parallelMergesortOnThird(arr, left, mid, temp, 0);
        }
        if (mid + 1 < right) {
            parallelMergesortOnThird(arr, mid + 1, right, temp, 0);
        }
    }
...
#pragma omp single
        {
            parallelMergesortOnThird(arr, 0, n - 1, temp, depthLimit);
        }
    }
...
void sortChunk(Chunk* chunk, int on_index) {
    ...
    if (on_index == 0) {
        parallelSortOnZero(chunk->records, n);
    }
    else if (on_index == 3) {
        parallelSortOnThird(chunk->records, n); 
    }
    else {
        perror("sortChunk: on_index not implemented in parallel mergesort");
        exit(EXIT_FAILURE);
    }
}

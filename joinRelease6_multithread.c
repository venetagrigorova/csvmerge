...
// Parallel Quicksort on fields[0]
static void parallelQuicksortOnZero(Record* arr, int left, int right, int depthLimit) {
    ...
    // Parallel tasks
    if (depthLimit > 0) {
    #pragma omp task shared(arr) firstprivate(left, j, depthLimit)
        {
            parallelQuicksortOnZero(arr, left, j, depthLimit - 1);
        }
    #pragma omp task shared(arr) firstprivate(i, right, depthLimit)
        {
            parallelQuicksortOnZero(arr, i, right, depthLimit - 1);
        }
    #pragma omp taskwait
    } else {
        // Sequential recursion
        if (left < j) {
            parallelQuicksortOnZero(arr, left, j, 0);
        }
        if (i < right) {
            parallelQuicksortOnZero(arr, i, right, 0);
        }
    }
}

// Wrapper function for parallel quicksort on fields[0]
void parallelSortOnZero(Record* arr, int n) {
    if (n <= 1) return;

    int depthLimit = 3;  // log2(8) = 3 for 8 cores

    // No parallel region here
    parallelQuicksortOnZero(arr, 0, n - 1, depthLimit);
}

// Parallel Quicksort on fields[3]
static void parallelQuicksortOnThird(Record* arr, int left, int right, int depthLimit) {
    if (left >= right) return;
    ...
    #pragma omp task shared(arr) firstprivate(left, j, depthLimit)
        {
            parallelQuicksortOnThird(arr, left, j, depthLimit - 1);
        }
    #pragma omp task shared(arr) firstprivate(i, right, depthLimit)
        {
            parallelQuicksortOnThird(arr, i, right, depthLimit - 1);
        }
    #pragma omp taskwait
    } else {
        // Sequential recursion
        if (left < j) {
            parallelQuicksortOnThird(arr, left, j, 0);
        }
        if (i < right) {
            parallelQuicksortOnThird(arr, i, right, 0);
        }
    }
}

// Wrapper function for parallel quicksort on fields[3]
void parallelSortOnThird(Record* arr, int n) {
    if (n <= 1) return;

    int depthLimit = 3;  // log2(8) = 3 for 8 cores

    // No parallel region here
    parallelQuicksortOnThird(arr, 0, n - 1, depthLimit);
}

// Updated sortChunk function
void sortChunk(Chunk* chunk, int on_index) {
    ...
    if (on_index == 0) {
        // Sort by fields[0] using parallel quicksort
        parallelSortOnZero(chunk->records, chunk->numRecords);
    }
    else if (on_index == 3) {
        // Sort by fields[3] using parallel quicksort
        parallelSortOnThird(chunk->records, chunk->numRecords);
    }
    else {

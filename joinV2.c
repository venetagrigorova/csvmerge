...
void mergeTwoRecords(const Record* left, const Record* right,
                        const Record* target) {
...
Table* joinTables(Table* left, Table* right, const char* outputFileName) {
    ...
    while(!left->endOfFile) {
        ...
        if(comparison == 0) {
            ...
            mergeTwoRecords(&left->currentRecord, &right->currentRecord, tempRecord);
            ...
            if(recordFitsBuffer(&left->currentRecord, left_on, buffer)) {
                ...
                for(int i = 0; i < buffer->numRecords; i++) {
                    mergeTwoRecords(&left->currentRecord,&buffer->records[i], tempRecord);
                    writeRecordToFile(tempRecord, output, numFieldsOutput);
                }
            }
